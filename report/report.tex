...
Even though no real attack exploiting Spectre has been made till date, there are compelling proofs of concept that show that modern processors have fundamental design flaws that can result in extremely sensitive data (such as encryption keys) being stolen. The risk is even greater in \textbf{cloud systems} where the isolation is broken by \textit{variant 2}. 

Intel is currently redesigning its processors to mitigate Spectre vulnerabilities. Several software patches have been released to block specific attacks, and some browsers have applied patches (the javascript attack used in this report does not work on the latest version of chrome). <Something about effects on performance>
...
\section{Next Steps}
Here are some of our failures which we are currently working on and will continue to work on the same till the deadline and since this has developed keep interest amongst us we may even continue even after that. Some of them are mentioned below
\begin{itemize}
	\item We were unable to experimentally demonstrate variant 2 of spectre. We are not able to bypass the isolation security between processes set up by the OS. We plan to look into this with rigour. We will look into the Google's project zero for the same.\cite{ProjectZero}
...
\begin{itemize}
		\item The experiment clearly demonstrates in a very modular fashion the exact control flow and implementation of how a Spectre attack variant1 can be carried out.
		\item The attack is based on very fundamental aspects/mechanism of modern day processors.
		
		\item Variant2 looks more severe as it can effect even a properly written victim code.
		 
		\item The suggested mitigations will lead to decrease in performance resulting in a tradeoff between security and performance which is clearly documented in the corresponding section
		\item In general Spectre is difficult to exploit which is evident from the fact that there has been no instance of such an attack till date.
		\item In the paper they have been very transparent in pointing out the flaws in the mitigations that has been provided

\end{itemize}
\section{Negative Aspects}
\begin{itemize}
	\item Spectre is very difficult to exploit.(Actually positive to modern day processor)
	\item Google's Project Zero\cite{ProjectZero} have worked very hard in presenting the POC's but it is actually very hard to use it in real life large scale effect.
	\item NetSpectre\cite{NETSPECTRE} Retrieves bit at very slow rate (60 bits/seconds) and any attempt to improve this data leak rate will make this attack detectable.
	\item Variant2 is out of scope for this project in terms of implementation/Experiment because it needs more knowledge on ROP and Buffer Overflow Attacks
\end{itemize}
\section{Fresh Ideas}
\begin{itemize}
	\item There are browsers which open new tabs in same process (different threads). Such browsers can be exploited using Spectre Variant1 to extract sensitive information.
\end{itemize}
